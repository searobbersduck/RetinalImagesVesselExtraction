...
class WholeImageTrainingDS(Dataset):
    '''
    ...
    def __init__(self, root, input_trans = None, target_trans=None):
        ...
        self.labels_root = os.path.join(root, 'ahe_mask')
        ...
        for index in images_list:
            basename = os.path.basename(index).split('.')[0].replace('_ahe', '_ahe_mask')
            if os.path.join(self.labels_root, basename+'.png') in labels_list:
                self.image_list.append(index)

        # with open('whole_images_label.txt','w') as f:
    ...
    def __getitem__(self, item):
        ...
        label_file = os.path.join(self.labels_root, os.path.basename(image_file).replace('_ahe.png', '_ahe_mask.png'))
        # print(image_file)
        ...
        image, x1, y1 = RandomCropImage(448)(image)
        label = label.crop((x1, y1, x1 + 448, y1 + 448))
